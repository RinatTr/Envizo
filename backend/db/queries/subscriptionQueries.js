...
const getSubscriptionsForAUser = (req, res, next) => {
  const userId = parseInt(req.params.user_id)
  db.any('SELECT subscriptions.goal_id as goal_id,goals.title, users.id as user_id, users.username, users.avatar_img FROM subscriptions JOIN users ON users.id = subscriptions.user_id JOIN goals ON goals.id = subscriptions.goal_id WHERE users.id = $1', userId)
    .then(subscripUser => {
      res.status(200).json({
