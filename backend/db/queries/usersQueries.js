...
const getActivityPerUser = (req, res, next) => {
  ...
  db.any('SELECT activity.id AS activity_id, type, username, goals.id AS goal_id, activity.time_stamp, users.id AS user_id, name, communities.id AS community_id, title FROM activity JOIN users ON users.id = activity.user_id JOIN communities ON communities.id = users.community_id JOIN subscriptions ON subscriptions.id = activity.subscription_id JOIN goals ON goals.id = subscriptions.goal_id WHERE users.id=$1 ORDER BY activity.time_stamp DESC', user_id)
    .then(data => {
      res.status(200)
      .json({
        status:'Success',
